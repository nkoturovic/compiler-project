...
DataRepresent Data::type_to_represent(
...
Data::Data(jbcoe::polymorphic_value<DataType> type, integer_t data)
    : type(std::move(type)) {
    m_data.emplace<integer_t>(data);
    this->fix_precision();
}
Data::Data(jbcoe::polymorphic_value<DataType> type, int data)
    : Data(std::move(type), static_cast<integer_t>(data)) {}
Data::Data(jbcoe::polymorphic_value<DataType> type, const char *data)
    : Data(std::move(type),
           static_cast<integer_t>(reinterpret_cast<std::intptr_t>(data))) {}
Data::Data(jbcoe::polymorphic_value<DataType> type, real_t data)
    : type(std::move(type)) {
    m_data.emplace<real_t>(data);
    this->fix_precision();
}

Data Data::do_bin_op(BinOpId op, const Data &lhs, const Data &rhs,
                     const jbcoe::polymorphic_value<DataType> &res_type) {
    if (res_type->id == DataTypeId::INVALID ||
        lhs.type->id == DataTypeId::INVALID ||
        rhs.type->id == DataTypeId::INVALID)
        return Data();
    ...
    if (lhs.type->id == DataTypeId::PTR && rhs.type->id <= DataTypeId::INT) {
        ...
        switch (op) {
            case BinOpId::PLUS:
                return Data(res_type, static_cast<integer_t>(
                                          o1 + o2 * ptt.points_to->size()))
                    .fix_precision();
            case BinOpId::MINUS:
                return Data(res_type, static_cast<integer_t>(
                                          o1 - o2 * ptt.points_to->size()))
    ...
    } else if (lhs.type->id <= DataTypeId::INT &&
               rhs.type->id == DataTypeId::PTR) {
        integer_t o1 = std::get<integer_t>(lhs.m_data);
        integer_t o2 = std::get<integer_t>(rhs.m_data);
    ...
    if (type_repr == DataRepresent::INTEGER) {
        ...
        switch (op) {
            ...
            case BinOpId::DIV:
                return Data(res_type, o1 / o2).fix_precision();
            // case BinOpId::MOD : return Data(res_type, o1 %
            // o2).fix_precision();
            case BinOpId::EQ:
                return Data(res_type, o1 == o2).fix_precision();
            case BinOpId::NEQ:
            ...
            case BinOpId::L_OR:
                return Data(res_type, o1 || o2).fix_precision();
            /*case BinOpId::B_AND : return Data(res_type, o1 &
             * o2).fix_precision();*/
            /*case BinOpId::B_OR : return Data(res_type, o1 |
             * o2).fix_precision();*/
            /*case BinOpId::B_XOR : return Data(res_type, o1 ^
             * o2).fix_precision();*/
            /*case BinOpId::SHL : return Data(res_type, o1 <<
             * o2).fix_precision();*/
            /*case BinOpId::SHR : return Data(res_type, o1 >>
             * o2).fix_precision();*/
            default:
                return Data();
        }
...
Data Data::do_un_op(UnOpId op, const Data &expr,
                    const jbcoe::polymorphic_value<DataType> &res_type) {
    ...
    if (type_repr == DataRepresent::INTEGER) {
        ...
        switch (op) {
            case UnOpId::PLUS:
                return Data(res_type, o).fix_precision();
            case UnOpId::MINUS:
                return Data(res_type, -o).fix_precision();
            case UnOpId::L_NOT:
                return Data(res_type, !o).fix_precision();
...
Data &Data::fix_precision() {
    ...
    if (repr == DataRepresent::INTEGER) {
        this->m_data.emplace<integer_t>(
            std::get<integer_t>(this->m_data) %
            static_cast<integer_t>(pow(2, this->type->size() * 8) - 1));
    } else if (repr == DataRepresent::REAL) {
        if (this->type->size() == sizeof(float))
            this->m_data.emplace<real_t>(
                static_cast<float>(std::get<real_t>(this->m_data)));
        else if (this->type->size() == sizeof(double))
            this->m_data.emplace<real_t>(
                static_cast<double>(std::get<real_t>(this->m_data)));
    }
    return *this;
}

Data Data::convert(const Data &from,
                   const jbcoe::polymorphic_value<DataType> &type) {
    Data to = from;
    to.type = type;

    if (from.type->id == to.type->id) return to;
    ...
    if (from_repr != to_repr) {
        if (from_repr == DataRepresent::INTEGER &&
            to_repr == DataRepresent::REAL)
            to.m_data.emplace<real_t>(std::get<integer_t>(to.m_data));
        else if (from_repr == DataRepresent::REAL &&
                 to_repr == DataRepresent::INTEGER)
    ...
    if (rep == DataRepresent::INTEGER) {
        ...
        if (data.type->id == DataTypeId::CHAR) {
            ...
            const PointerType &pt =
                dynamic_cast<const PointerType &>(*data.type);
            if (pt.points_to->id == DataTypeId::CHAR) {
                const char *str = reinterpret_cast<const char *>(
                    static_cast<intptr_t>(std::get<integer_t>(data.m_data)));
                out << str;
            }
        }
    } else if (rep == DataRepresent::REAL) {
        real_t val = std::get<real_t>(data.m_data);
        if (data.type->id == DataTypeId::DOUBLE) out << val;
