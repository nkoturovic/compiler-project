...
std::size_t BasicType::size() const { return m_basic_type_to_size_table.at(this->id); }
std::string BasicType::str() const { return m_basic_type_to_string_table.at(this->id); }

PointerType::PointerType(jbcoe::polymorphic_value<DataType> points_to) : DataType(DataTypeId::PTR), points_to(std::move(points_to)) { }
bool PointerType::operator==(const PointerType &rhs) const {
    if (this->id == rhs.id && *this->points_to == *rhs.points_to)   // TODO: maybe bug
        return true;
    return false;
}
