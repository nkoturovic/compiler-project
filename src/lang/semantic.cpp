...
using poly_type = jbcoe::polymorphic_value<cpl::lang::DataType>;

namespace cpl::lang {

jbcoe::polymorphic_value<DataType> Semantic::binop_check(
    BinOpId op_id, const jbcoe::polymorphic_value<DataType> &op1_t,
    const jbcoe::polymorphic_value<DataType> &op2_t) {
    ...
    if (op1_t->id <= DataTypeId::DOUBLE || op2_t->id <= DataTypeId::DOUBLE) {
        ...
        return poly_type(std::max(std::max(op1_basic_t, op2_basic_t),
                                  BasicType(DataTypeId::INT)));
    }

    return poly_type(InvalidType());
}

jbcoe::polymorphic_value<DataType> Semantic::unop_check(
    UnOpId op_id, const jbcoe::polymorphic_value<DataType> &type) {
    if (op_id == UnOpId::INVALID || type->id == DataTypeId::INVALID ||
        type->id == DataTypeId::VOID)
        return poly_type(InvalidType());
