...
void SymbolTable::begin_scope() {
    m_scopes.push_back(
        std::unordered_map<std::string, structs::TypeValuePair>());
}

void SymbolTable::end_scope() {
    if (m_scopes.size() == 0) throw SymbolException("No scopes to end");
    m_scopes.resize(m_scopes.size() - 1);
}

void SymbolTable::push_scope() {
    if (m_scopes.size() == 0) throw SymbolException("No scopes to push");
...
unsigned SymbolTable::num_of_scopes() { return m_scopes.size(); }

std::optional<structs::TypeValuePair> SymbolTable::get_var_curr_scope_only(
    std::string id) {
    if (m_scopes.size() == 0) return {std::nullopt};

    auto it = m_scopes[m_scopes.size() - 1].find(id);
    if (it != m_scopes[m_scopes.size() - 1].end()) return it->second;

    return {std::nullopt};
}

std::optional<structs::TypeValuePair> SymbolTable::get_variable(
    std::string id) {
    if (m_scopes.size() == 0) return {std::nullopt};
...
std::optional<structs::Error> SymbolTable::declare_function(
    std::string id, structs::LocProtoFuncTriple lpf) {
...
std::optional<structs::Error> SymbolTable::define_function(
    std::string id, structs::LocProtoFuncTriple lpf) {
    std::optional<structs::Error> opt_err = declare_function(id, lpf);
    if (opt_err.has_value()) return opt_err;

    auto it = m_defined_functions.find(id);
    if (it != m_defined_functions.end()) {
        std::stringstream err_ss;
        err_ss << "Function " << id << " already defined at " << it->second.loc;
        return structs::Error{lpf.loc, err_ss.str()};
    }
...
std::optional<structs::LocProtoFuncTriple> SymbolTable::get_function(
    std::string id) {
    auto it = m_declared_functions.find(id);
    if (it == m_declared_functions.end())
        return {std::nullopt};
